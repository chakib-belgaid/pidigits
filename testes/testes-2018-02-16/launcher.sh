...
do  
    ...
    p=10000
    for i0 in {1..$1} # we proceed with a step of 5000 starting with 10000 $1 is the number of iterations 
    do 
        k=1
        for i1 in {0..3} #2^x loops inside the program
        do
            for l in {1..10} # instaces of the program ( just to be sure we have the right value )   
            do  
            docker run $i $p $k >> "$ss.$p.$k.power" #<algo>.<language>.<compilflag>.<param>.<inerllo>.power
            #echo -n ;
            #echo $i - ${j}0000
            done
            k=$((k*2))
